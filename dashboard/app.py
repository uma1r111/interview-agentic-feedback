        ...
        with col_wizard:
            ...
            if step2_locked:
                ...
                for doc_key, label, types, field_name, hint in done_docs:
                    st.markdown(
                        f"<div style='border:1px solid #E5DEF0;border-radius:8px;padding:10px 14px;margin-bottom:8px'>",
                        unsafe_allow_html=True
                    )
                    ...
                    with col_lbl:
                        st.markdown(f"**✅ {label}**")
                        st.caption(hint)
                        st.markdown("<span style='color:#0F7A3D;font-size:0.75rem'>✔ Saved — locked</span>", unsafe_allow_html=True)
                    with col_up:
                ...
                if pending_docs:
                    with st.form(f"upload_form_{cid}"):
                        ...
                        for doc_key, label, types, field_name, hint, in_replace in pending_docs:
                            icon = "🔄" if in_replace else "⬜"
                            border_color = "#5B2A8E" if in_replace else "#E5DEF0"
                            st.markdown(
                                f"<div style='border:1px solid {border_color};border-radius:8px;padding:10px 14px;margin-bottom:8px'>",
                                unsafe_allow_html=True
                            )
                        ...
                        if submitted:
                            ...
                            if not files_to_upload:
                                ...
                                if ok:
                                    mcq_msg = ""
                                    if result and result.get("mcq_score") is not None:
                                        mcq_msg = f" MCQ Score extracted: **{result.get('mcq_score')}/5.0**"
                                    st.success(f"✅ Documents saved successfully!{mcq_msg}")
